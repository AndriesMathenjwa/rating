...
const Verification = () => {
  ...
  const handleRePrint = async () => {
    ...
    try {
      const response = await fetch(`https://script.google.com/macros/s/AKfycbwTNSk1pSouCwphaKEHw0EhTlUQAulXbX9SOvFn2OBMbfY_a6Jt42Si3l2JoHr-ix3q/exec?idNumber=${idNumber}`);
      if (!response.ok) {
  ...
  const handleDataResponse = (response) => {
    if (response.status === 'success') {
      Swal.fire({
        title: "Success",
        text: "Verification Successful. Proceed with ticket reprint.",
        icon: "success",
        customClass: {
    ...
    <React.Fragment>
      ...
      >
        <Box sx={{ textAlign: "center", width: { xs: '90%', sm: '70%', md: '60%', lg: '50%' } }}>
          <Typography variant="h4" sx={{ mb: 3, fontWeight: 'bold' }}>Ticket Verification</Typography>
          <TextField
            id="id-number"
            label="Enter ID number to Verify"
            variant="outlined"
            fullWidth
            sx={{
              mb: 2,
